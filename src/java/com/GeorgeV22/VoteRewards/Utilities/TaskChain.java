 ...
 * Async and Sync tasks without deeply nested callbacks, passing the response of
 * the previous task to the next task to use.
 *
 * Usage example: TaskChain.newChain() .add(new TaskChain.AsyncTask {}) .add(new
 * TaskChain.Task {}) .add(new AsyncTask {}) .execute();
 */
public class TaskChain {
	/**
	 * Utility helpers for Task returns. Changes the behavior of the Chain when
	 * these are returned.
	 */
	...
	private abstract static class BaseTask<R, A> {
		...
		/**
		 * Called internally by Task Chain to facilitate executing the task and then the
		 * next task.
		 * 
		 * @param chain
		 */
		...
		/**
		 * Only to be used when paired with return this.async(); Must be called to
	 ...
	 *
	 * A Generic task simply does not care about Previous Return or return anything
	 * itself.
	 *
